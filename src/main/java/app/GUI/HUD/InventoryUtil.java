...
public class InventoryUtil {
    ...
    private final ArrayList<BaseObject> inventory;
    ...
    private BaseObject currentItem = new BaseObject();
    ...
    public InventoryUtil(PlayerUtil ctx, int inventorySize) {
        ...
        for (int i = 0; i < inventorySize; i++) {
            inventory.add(i, new BaseObject());
        }
    }
    ...
    public int getItemSize(BaseObject Base_Structure) {
    ...
    public BaseObject getCurrentItem() {
    ...
    public BaseObject popCurrentItem() {
     ...
     * @return
     */
    public BaseObject popItem(int index) {
        if (index < 0 || index >= inventorySize) {
            Log.d(TAG, "The given index is not in range");
            return new BaseObject();
        }

        BaseObject tmp = inventory.get(index);
        Log.d(TAG, "POP ITEM " + tmp.getScaleX() + " " + tmp.getScaleY() + " " + tmp.getScaleZ());

        Log.d(TAG, "/////////// " + tmp.getShape());
        if (tmp.getProps().getPROPERTY_ITEM_TAG() == ObjectProperties.UNDEFINED_TAG) {
            ...
            inventory.set(index, new BaseObject());
        } else {
     ...
     * @return
     */
    public int popItem(BaseObject item) {
     ...
     * @return
     */
    public BaseObject popItem(int index, int size) {
        if (size < 1) {
     ...
     * @return
     */
    public int popItem(BaseObject item, int size) {
        if (size < 1) {
            Log.d(TAG, "Size must be greater than 0");
            return -1;
        }
    ...
    public void addItem(BaseObject item) {
        ...
        if (indexes.containsKey(item.getProps().getPROPERTY_ITEM_TAG())) {
            ...
            for (int i = 0; i < inventory.size(); i++) {
                if (inventory.get(i).getProps().getPROPERTY_ITEM_TAG() == ObjectProperties.UNDEFINED_TAG) {
                    addItem(i, item);
                    break;
                }
            }
    ...
    public void addItem(BaseObject item, int size) {
    ...
    public void addItem(int index, BaseObject item) {
        ...
        if (inventory.get(index).getProps().getPROPERTY_ITEM_TAG() == ObjectProperties.UNDEFINED_TAG) {
    ...
    public void addItem(int index, BaseObject item, int size) {
    ...
    public void swapItems(int index1, int index2) {
        ...
        // item at index1
        BaseObject item1 = inventory.get(index1);
        String item1Tag = item1.getProps().getPROPERTY_ITEM_TAG();

        // item at index2
        BaseObject item2 = inventory.get(index2);
    ...
    public void print() {
        ...
        for (int i = 0; i < inventory.size(); i++) {
            BaseObject item = inventory.get(i);
            int size;

            if (item.getProps().getPROPERTY_ITEM_TAG() == ObjectProperties.UNDEFINED_TAG) size = 0;
